...
app.get('/fare', async (req, res, next) => {
    try {
        ...
        +'FROM tickets'+'INNER JOIN users ON tickets.user = users.id AND users.id = ?'+'INNER JOIN trains ON tickets.train = trains.id'+'INNER JOIN types ON trains.type = types.id';
...
app.get('/train/status', async (req,res,next) => {
    try {
        ...
        +'FROM tickets'+'INNER JOIN trains ON trains.id =tickets.train AND trains.id=?'+'INNER JOIN types ON trains.type = types.id';
        const {occupied, maximum}=(await runQuery(sql,[tid]))[0];
        const checkSeatsleft= occupied<maximum;
        return res.send('Train ${tid} is ${checkSeatsleft ?} sold out');
    } catch (err) {
        console.error(err);
        return res.sendStatus(500);
    }
